...
if ( ! class_exists( 'WC_Product_Woosb' ) && class_exists( 'WC_Product' ) ) {
	class WC_Product_Woosb extends WC_Product {
				...
				if ( $discount_amount || $discount_percentage ) {
					...
					if ( $items = $this->get_items() ) {
						foreach ( $items as $item ) {
							...
							$_price = WPCleverWoosb_Helper::woosb_get_price( $_product, 'min', $has_discount ) * $item['qty'];
		...
		public function get_manage_stock( $context = 'view' ) {
			...
			if ( ( $items = $this->get_items() ) && ! $this->is_optional() && ( $exclude_unpurchasable !== 'yes' ) ) {
				foreach ( $items as $item ) {
					$_product = wc_get_product( $item['id'] );
		...
		public function get_stock_quantity( $context = 'view' ) {
			...
			if ( ( $items = $this->get_items() ) && ! $this->is_optional() && ( $exclude_unpurchasable !== 'yes' ) ) {
				...
				foreach ( $items as $item ) {
					...
					if ( ! $_product || $_product->is_type( 'woosb' ) || ( $_product->get_stock_quantity() === null ) ) {
						continue;
					}
		...
		public function get_backorders( $context = 'view' ) {
			...
			if ( ( $items = $this->get_items() ) && ! $this->is_optional() && ( $exclude_unpurchasable !== 'yes' ) ) {
				...
				foreach ( $items as $item ) {
					...
					if ( ! $_product || $_product->is_type( 'woosb' ) || ! $_product->get_manage_stock() ) {
						continue;
					}
		...
		public function get_items() {
			...
			if ( $ids = get_post_meta( $product_id, 'woosb_ids', true ) ) {
				$items = explode( ',', $ids );
