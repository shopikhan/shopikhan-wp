...
if ( ! class_exists( 'WPCleverWoosb' ) && class_exists( 'WC_Product' ) ) {
	class WPCleverWoosb {
		...
		function woosb_add_to_cart_validation( $passed, $product_id ) {
			...
			if ( $_product && $_product->is_type( 'woosb' ) && ( $ids = get_post_meta( $product_id, 'woosb_ids', true ) ) ) {
				...
				if ( $items = $this->woosb_get_bundled( 0, $ids ) ) {
		...
		function woosb_add_to_cart_items( $items, $cart_item_key, $product_id, $quantity ) {
			...
			if ( is_array( $items ) && ( count( $items ) > 0 ) ) {
				foreach ( $items as $item ) {
					...
					$_variation_id = 0;
					$_variation    = array();
					$_price        = WPCleverWoosb_Helper::woosb_get_price( $_product, 'min', $has_discount );
				...
				if ( $product && $product->is_type( 'woosb' ) && ( $items = $product->get_items() ) ) {
					...
					foreach ( $items as $item ) {
						...
						if ( $fixed_price ) {
							...
							$_price = (float) ( 100 - $discount_percentage ) * WPCleverWoosb_Helper::woosb_get_price( $_product, 'min', true ) / 100;
							$_product->set_price( $_price );
						}

						// add bundled products
						$order_item_id = $order->add_product( $_product, $item['qty'] * $quantity );
						...
						$order_item->add_meta_data( '_woosb_parent_id', $product_id, true );
						$order_item->save();
					}
		...
		function woosb_product_data_panels() {
			...
			if ( get_post_meta( $post_id, 'woosb_ids', true ) ) {
				$ids_str = get_post_meta( $post_id, 'woosb_ids', true );
			} elseif ( isset( $_GET['woosb_ids'] ) ) {
				$ids_str = implode( ',', explode( '.', $_GET['woosb_ids'] ) );
			}
                            ...
                            <div class="w100">
                                ...
                                <div id="woosb_selected" class="woosb_selected">
                                    <ul>
										<?php
										if ( ! empty( $ids_str ) ) {
											$items = $this->woosb_get_bundled( 0, $ids_str, false );
				...
				if ( ! $product->is_fixed_price() ) {
					...
					if ( $product->is_optional() ) {
						...
						foreach ( $items as $item ) {
							...
							if ( $_product ) {
								$prices[] = WPCleverWoosb_Helper::woosb_get_price_to_display( $_product, 1, 'min', $has_discount );
							}
						}
						...
						foreach ( $items as $item ) {
							...
							if ( $_product ) {
								$min_price += WPCleverWoosb_Helper::woosb_get_price_to_display( $_product, $item['qty'], 'min', $has_discount );
								$max_price += WPCleverWoosb_Helper::woosb_get_price_to_display( $_product, $item['qty'], 'max', $has_discount );
							}
						}
						...
						foreach ( $items as $item ) {
							...
							if ( $_product ) {
								$_price = WPCleverWoosb_Helper::woosb_get_price_to_display( $_product, $item['qty'], 'min', $has_discount );
                ...
                <div class="woosb-products"
					...
					foreach ( $items as $item ) {
                        ...
                        <div class="woosb-product"
                             ...
                             data-price="<?php echo esc_attr( WPCleverWoosb_Helper::woosb_get_price_to_display( $_product, 1, 'min', $has_discount ) ); ?>"
                                ...
                                <div class="woosb-price">
                                    <div class="woosb-price-ori">
										...
										if ( ! $product->is_fixed_price() && ( $discount_percentage = $product->get_discount() ) ) {
											$show_new_price = true;
											$_product_price = WPCleverWoosb_Helper::woosb_get_price( $_product, 'min', true ) * ( 100 - $discount_percentage ) / 100;
										} else {
											$show_new_price = false;
											$_product_price = WPCleverWoosb_Helper::woosb_get_price( $_product, 'min', false );
										}

										switch ( get_option( '_woosb_bundled_price', 'price' ) ) {
											case 'price':
												if ( $show_new_price ) {
													$_price = wc_format_sale_price( WPCleverWoosb_Helper::woosb_get_price_to_display( $_product, 1, 'min', $has_discount ), wc_get_price_to_display( $_product, array( 'price' => $_product_price ) ) );
												} else {
											...
											case 'subtotal':
												if ( $show_new_price ) {
													$_price = wc_format_sale_price( WPCleverWoosb_Helper::woosb_get_price_to_display( $_product, $item['qty'], 'min', $has_discount ), wc_get_price_to_display( $_product, array(
														'price' => $_product_price,
														'qty'   => $item['qty']
													) ) );
												} else {
													$_price = wc_price( wc_get_price_to_display( $_product, array( 'qty' => $item['qty'] ) ) );
												}
