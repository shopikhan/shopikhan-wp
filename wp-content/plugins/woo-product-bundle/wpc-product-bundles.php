...
! defined( 'WOOSB_VERSION' ) && define( 'WOOSB_VERSION', '4.0.4' );
		...
		if ( ! class_exists( 'WPCleverWoosb' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWoosb {
				function __construct() {
					...
					add_action( 'woocommerce_process_product_meta_woosb', array(
						$this,
						'woosb_save_option_fields'
					) );
					...
					add_filter( 'woocommerce_cart_item_subtotal', array(
						$this,
						'woosb_cart_item_subtotal'
					), 10, 2 );
					...
					add_filter( 'woocommerce_cart_contents_weight', array(
						$this,
						'woosb_cart_contents_weight'
					), 99, 1 );
				...
				function woosb_get_search_results() {
					...
					if ( is_array( $added_ids ) && count( $added_ids ) > 2 ) {
						echo '<ul><span>Please use the Premium Version to add more than 3 products to the bundle & get the premium support. Click <a href="https://wpclever.net/downloads/woocommerce-product-bundle?utm_source=pro&utm_medium=woosb&utm_campaign=wporg" target="_blank">here</a> to buy, just $29!</span></ul>';
						die();
					}
					...
					if ( $woosb_query->have_posts() ) {
						...
						while ( $woosb_query->have_posts() ) {
							...
							if ( $woosb_product->is_type( 'variable' ) ) {
								// show all children
								$woosb_children = $woosb_product->get_children();

								if ( is_array( $woosb_children ) && count( $woosb_children ) > 0 ) {
									foreach ( $woosb_children as $woosb_child ) {
				...
				function woosb_meta_boxes_content() {
					...
					if ( $post_id > 0 ) {
						...
						if ( $woosb_product && ! $woosb_product->is_type( 'woosb' ) ) {
							?>
                            <p><?php esc_html_e( 'Update the price of all bundles containing this product. The completion time for this process depends on the number of bundles you have in your store.', 'woo-product-bundle' ); ?></p>
                            ...
                            <p><?php esc_html_e( 'This tool is for updating the price of all bundles containing the modified product. It is invalid for a bundle.', 'woo-product-bundle' ); ?></p>
						<?php }
					} else { ?>
						...
						if ( ! $product->is_fixed_price() ) {
							...
							if ( $product->is_optional() ) {
								...
								foreach ( $woosb_items as $woosb_item ) {
									...
									if ( $woosb_product ) {
										if ( $woosb_product->is_type( 'variable' ) ) {
											$prices[] = (float) wc_get_price_to_display( $woosb_product, array(
												'price' => $woosb_product->get_variation_price( 'min' )
											) );
										} else {
											$prices[] = (float) wc_get_price_to_display( $woosb_product );
										}
									}
								}
								...
								foreach ( $woosb_items as $woosb_item ) {
									...
									if ( $woosb_product ) {
										if ( $woosb_product->is_type( 'variable' ) ) {
											$min_price += (float) wc_get_price_to_display( $woosb_product, array(
												'qty'   => $woosb_item['qty'],
												'price' => $woosb_product->get_variation_price( 'min' )
											) );
											$max_price += (float) wc_get_price_to_display( $woosb_product, array(
												'qty'   => $woosb_item['qty'],
												'price' => $woosb_product->get_variation_price( 'max' )
											) );
										} else {
											$min_price += (float) wc_get_price_to_display( $woosb_product, array( 'qty' => $woosb_item['qty'] ) );
											$max_price += (float) wc_get_price_to_display( $woosb_product, array( 'qty' => $woosb_item['qty'] ) );
										}
									}
								}
								...
								foreach ( $woosb_items as $woosb_item ) {
									...
									if ( $woosb_product ) {
										if ( $woosb_product->is_type( 'variable' ) ) {
											$price += (float) wc_get_price_to_display( $woosb_product, array(
												'qty'   => $woosb_item['qty'],
												'price' => $woosb_product->get_variation_price( 'min' )
											) );
										} else {
											$price += (float) wc_get_price_to_display( $woosb_product, array( 'qty' => $woosb_item['qty'] ) );
										}
									}
								}
                        ...
                        <div class="woosb-products"
							...
							<?php foreach ( $woosb_items as $woosb_item ) {
                                ...
                                <div class="woosb-product"
									...
									<?php
									do_action( 'woosb_before_item', $woosb_product );
									...
									do_action( 'woosb_after_item', $woosb_product );
									?>
                                </div>
								<?php
								$woosb_count ++;
				...
				function woosb_display_post_states( $states, $post ) {
					if ( 'product' == get_post_type( $post->ID ) ) {
						if ( ( $product = wc_get_product( $post->ID ) ) && $product->is_type( 'woosb' ) ) {
							...
							$states[] = apply_filters( 'woosb_post_states', '<span class="woosb-state">' . sprintf( esc_html__( 'Bundle (%s)', 'woo-product-bundle' ), $woosb_count ) . '</span>', $woosb_count, $product );
						}
					}
